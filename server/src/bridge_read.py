...
async def get_install(request: Request, name: str = Query(None)):
    ...
    script = (
        ...
        '    sudo apt install -y -qq tmux >/dev/null\n'
        '  elif command -v dnf >/dev/null 2>&1; then\n'
        '    sudo dnf install -y -q tmux >/dev/null\n'
        '  elif command -v yum >/dev/null 2>&1; then\n'
        '    sudo yum install -y -q tmux >/dev/null\n'
        '  elif command -v brew >/dev/null 2>&1; then\n'
        '    brew install tmux >/dev/null\n'
        '  else\n'
        '    echo "\\033[0;31mError: tmux not found and no supported package manager detected.\\033[0m" >&2\n'
        '    exit 1\n'
        '  fi\n'
        '  if ! command -v tmux >/dev/null 2>&1; then\n'
        '    echo "\\033[0;31mError: tmux installation failed.\\033[0m" >&2\n'
        '    exit 1\n'
        '  fi\n'
        'fi\n'
        '\n'
