...
def _windows_install_script(url, server, api_key, name):
    ...
    return "\n".join([
        ...
        "$runningTask = Get-ScheduledTask -TaskName 'Baton Bridge'",
        "if ($runningTask.State -ne 'Running') {",
        "  $taskResult = (Get-ScheduledTaskInfo -TaskName 'Baton Bridge').LastTaskResult",
        ...
        "  throw \"Baton Bridge failed to stay running (task result $taskResult). Log: $logTail\"",
        "}",
