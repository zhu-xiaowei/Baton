...
(function () {
  ...
  window.renderUserBubble = function (msg) {
    ...
    const images = Array.isArray(msg.content)
      ...
      : [];
    const imgHtml = images.map(b =>
      `<div class="img-placeholder" data-key="${esc(b.key)}">...</div>`
    ).join('');

    if (!text.trim() && !imgHtml) return '';
    const sep = (text.trim() && imgHtml) ? '<br>' : '';
    return `<div class="msg-user">
      <div class="msg-text">${esc(text)}${sep}${imgHtml}</div>
